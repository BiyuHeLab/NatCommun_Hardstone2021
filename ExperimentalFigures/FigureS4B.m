function FigureS4B
% Output tables containing interlobe statistics when only using connections
% where one electrode is "switch" or "maintain"
load('Data.mat');
...
for i_image = 1:length(images)
    ...
    notSigElectrodes = Data.Subjects.SwitchMaintainAnalysis.(imageName).pValue(selectedElectrodes) >= pThreshold;
    ...
    Rank_PreferredMinusPreferred_ToFrom(notSigElectrodes,notSigElectrodes) = nan; %remove connections where neither electrode is a switch or maintain selective electrode
...
disp('FaceVase Preferred > NonPreferred');
disp('%Difference');
disp(percentageDifference_FaceVasePreferred);
disp('log10(pValue)');
disp(pValue_FaceVasePreferred)

disp('FaceVase NonPreferred > Preferred');
disp('%Difference');
...
disp('Cube Preferred > NonPreferred');
disp('%Difference');
disp(percentageDifference_CubePreferred);
disp('log10(pValue)');
disp(pValue_CubePreferred)

disp('Cube NonPreferred > Preferred');
disp('%Difference');
